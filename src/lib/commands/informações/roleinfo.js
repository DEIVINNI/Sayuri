      ...
      embed: msg.embed(msg.author, false)
        ...
        .setDescriptionArray([
          [
            ...
            `${msg.emoji.bot.seta}Permiss${role.hasPermission('ADMINISTRATOR') ? 'ão' : 'ões'}: ${role.hasPermission('ADMINISTRATOR') ? 'Administrador' : Object.entries(role.serialize()).filter(([, allowed]) => allowed).map(([perm]) => msg.perms[perm]).join(' `|` ').replace('Ler mensagens `|` ', '').replace('Emojis externos `|` ', '').replace('Gerenciar cargos ou permissões `|` ', '')}.`,
          ],
        ]),
    });
  },
