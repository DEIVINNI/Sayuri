...
export function run({ msg, args }) {
  const member = msg.getMember(args.join(' '), 0, true);
    ...
    embed: msg.embed(msg.author, false, { thumbnail: { url: member.user.displayAvatarURL } })
      ...
      .setDescriptionArray([
        [
          ...
          `${msg.emoji.bot.seta}Cargos [\`${member.roles.size - 1}\`]: ${member.roles.array().slice(1).sort((a, b) => a.comparePositionTo(b)).reverse()
            .map((r) => r)
            .join(', ')}`,
        ], [
          `${msg.emoji.bot.seta}Permiss${member.hasPermission('ADMINISTRATOR') ? 'ão' : 'ões'} (cargo mais alto): ${member.hasPermission('ADMINISTRATOR') ? 'Administrador'
            : Object.entries(member.highestRole.serialize()).filter(([, allowed]) => allowed).map(([perm]) => msg.perms[perm]).join(' `|` ')
