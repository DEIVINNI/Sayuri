export function run({ msg, bot, args }) {
  ...
  if (!args[0]) {
    ...
    categories.forEach((category) => {
      ...
      try {
        embed.addField(`${msg.emoji.bot.seta} ${category !== 'nsfw' ? msg.firstUpperCase(category) : 'NSFW'} [${dir.size}]:`, `${(!msg.channel.nsfw && category === 'nsfw') ? '*Você precisa estar em um chat `NSFW` para ver estes comandos*' : dir.map((c) => `\`${c.help.name}\``).join(', ')}`);
      } catch (e) {
        console.log(e.stack);
      }
    });
    msg.channel.send(`<:cmd_:586617374141186097> \`|\` Comandos da \`${bot.user.username}\``, {
      embed: embed.setTimeFooter(`${msg.config.prefix[0]}help <categoria> ou ${msg.config.prefix[0]}<comando> --help para mais informações`, msg.author.displayAvatarURL),
    });
  }
  if (args[0]) {
